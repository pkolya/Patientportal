...
wb = openpyxl.load_workbook('D:/OrthoASC/source/OrthoASC_04_InsertProcedureOrdersResults.xlsx')
ws = wb.active
cols = ws.max_column
print(cols)
ws.cell(1, cols+1).value = "procedure_name1"
...
query="""select procedure_name,sort_order,class as cs,activity_type,category,description FROM ngkbm_proc_activity_items_ AS npa
 INNER JOIN ngkbm_proc_mstr_ npm ON npa.practice_id = npm.practice_id AND npm.procedure_id = npa.proc_id 
 where npa.specialty='"+specialty+"'
  AND npa.practice_id='0001' AND class in ('General Procedure Order','Medication Procedure Order') order by 1,2"""
...
for x in range(0,len(onerow)):
    ...
    # #print(ws.max_row)
    # print(cols)
wb.save('D:/OrthoASC/Results/OrthoASC_04_InsertProcedureOrdewnew1.xlsx')
